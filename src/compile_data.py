...
def sent1_data_compile():
    ...
    sent1_df = prep_folder.joinpath("sent1.parquet")
    df_collector = []
    
    for folder in sent_1_folders:
    ...
    final_df = pd.concat(df_collector, axis=0)
    # .fillna(0)
    ...
    # df_new = df_new.sort_values(["date","lat","long"])

    # df_new[df_new['dups']].to_csv("test.csv")

def sent2_data_compile():

    sent2_df = prep_folder.joinpath("sent2.parquet")
    df_collector = []
    
    for folder in sent_2_folders:
    ...
    final_df.to_parquet(sent2_df, index=False)

    df_new = pd.read_parquet(sent2_df)
    # df_new['dups']= df_new.duplicated(["date","lat","long"], keep=False )
    # df_new = df_new.sort_values(["date","lat","long"])
